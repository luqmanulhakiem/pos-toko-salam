                                        ...
                                        <select class="form-control" name="id_kategori" id="">
                                            ...
                                            @foreach ($kategori as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $data->id_kategori == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="mb-3">
                                        <label for="nameInput" class="form-label">Hargal Modal</label>
                                        <input type="text" name="cogs" value="{{ old('cogs', $data->cogs) }}"
                                            class="form-control" id="nameInput" aria-describedby="emailHelp" required />
                                    </div>
                                    <div class="mb-3">
                                        <label for="nameInput" class="form-label">Hargal Jual</label>
                                        <input type="text" name="price_sell"
                                            value="{{ old('price_sell', $data->price_sell) }}" class="form-control"
